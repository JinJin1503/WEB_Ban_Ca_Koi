...
namespace KoiFarmShop.Services.Implementations
{
    public class CartService : ICartService
    {
        ...
        public async Task AddCartItemToCartAsync(
            int customerId,
            int koiId,
            int quantityKoi,
            int quantityBatch,
        ...
        {
            // Chặn số lượng âm
            if (quantityKoi < 0 || quantityBatch < 0)
            {
                throw new Exception("Quantity cannot be negative.");
            }

            // Nếu cả 2 đều bằng 0 thì cũng không hợp lệ
            if (quantityKoi == 0 && quantityBatch == 0)
            {
                throw new Exception("At least one quantity must be greater than 0.");
            }
